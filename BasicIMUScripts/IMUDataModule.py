...
class IMUDataModule:
    def __init__(self, address = 105):
        i2c = board.I2C()  # uses board.SCL and board.SDA
        self.icm = adafruit_icm20x.ICM20948(i2c, address)
    ...
    def poll_imu_data(self):
        ...
        total_acceleration = tuple(
            map(sum, zip(acceleration, self.average_acceleration * self.length)))
        self.average_acceleration = tuple(
            val/self.length + 1 for val in total_acceleration)

        total_gyro = tuple(
            map(sum, zip(gyro, self.average_gyro * self.length)))
        self.average_gyro = tuple(val/self.length + 1 for val in total_gyro)

        total_magnetic = tuple(
            map(sum, zip(magnetic, self.average_magnetic * self.length)))
        self.average_magnetic = tuple(
            val/self.length + 1 for val in total_magnetic)

        total_angle = tuple(
            map(sum, zip(angle, self.average_angle * self.length)))
